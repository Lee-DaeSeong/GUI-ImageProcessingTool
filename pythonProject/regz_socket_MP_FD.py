...
def all(qwe):
    ...
    class gaze_redirection_system:
        ...
        def flx_gaze(self, frame, gray, detections, shared_v, lock, pixel_cut=[3, 4], size_I=[48, 64]):
            ...
            for k, bx in enumerate(detections):
                ...
                (R_le_LT[1] + pixel_cut[1]):(R_le_LT[1] + size_le_ori[1] - pixel_cut[1])] = LE_infer[pixel_cut[0]:(
                        -1 * pixel_cut[0]), pixel_cut[1]:-1 * (pixel_cut[1])] * 255
                frame[(R_re_LT[0] + pixel_cut[0]):(R_re_LT[0] + size_re_ori[0] - pixel_cut[0]),
                (R_re_LT[1] + pixel_cut[1]):(R_re_LT[1] + size_re_ori[1] - pixel_cut[1])] = RE_infer[pixel_cut[0]:(
                        -1 * pixel_cut[0]), pixel_cut[1]:-1 * (pixel_cut[1])] * 255
        ...
        def run(self, shared_v, lock):
            ...
            cv2.moveWindow(conf.uid, int(Rs[0] / 2) - int(size_window[0] / 2),
    ...
    def main():
        ...
        vs_thread = Thread(target=video_receiver, args=(conf.recver_port, l))
        # vs_thread = Thread(target=video_receiver, args=(v,l))
        vs_thread.start()
        time.sleep(1)
        gz_thread = Thread(target=gaze_redirection_system, args=(conf.recver_port, l))
        # gz_thread = Thread(target=gaze_redirection_system, args=(v,l))
        gz_thread.start()
        vs_thread.join()
        gz_thread.join()
